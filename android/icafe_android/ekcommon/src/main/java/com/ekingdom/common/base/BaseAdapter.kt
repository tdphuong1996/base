...
abstract class BaseAdapter<D, VH: BaseViewHolder<D>>(val layoutRes: Int, private val _arrData: MutableList<D> = ArrayList()) :
    RecyclerView.Adapter<VH>() {
    val arrData: MutableList<D>
        get() = _arrData

    val TYPE_ITEM_NORMAL: Int = 0
    ...
    var isLoadMore: Boolean = false
        set(value) {
            if (field != value) {
                field = value
                if (value)
                    notifyItemInserted(arrData.size)
                else
    ...
    override fun getItemViewType(position: Int): Int {
        return if (isLoadMore && position >= arrData.size) {
            TYPE_ITEM_LOAD_MORE
        } else
            TYPE_ITEM_NORMAL
    }
    ...
    override fun getItemCount(): Int {
        return _arrData.size + if (isLoadMore) 1 else 0
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
        return if (viewType == TYPE_ITEM_NORMAL)
            ...
            LoadMoreViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.rcv_item_loadmore, parent, false)) as VH
    }
